...
const annotationsAtFrame = (annotations, frame) => {
  const exact = annotations.filter((item) => item.frame_id === frame)
  const tracks = new Map()
  annotations.filter((item) => item.type === 'reid' && item.track_id && Number.isInteger(item.frame_id)).forEach((item) => {
    if (!tracks.has(item.track_id)) tracks.set(item.track_id, [])
    tracks.get(item.track_id).push(item)
  })
  const occupied = new Set(exact.map((item) => item.track_id).filter(Boolean))
  const interpolated = []
  tracks.forEach((items, trackId) => {
    if (occupied.has(trackId)) return
    items.sort((a, b) => a.frame_id - b.frame_id)
    const before = [...items].reverse().find((item) => item.frame_id < frame)
    const after = items.find((item) => item.frame_id > frame)
    if (!before || !after || before.points.length !== 2 || after.points.length !== 2) return
    const ratio = (frame - before.frame_id) / (after.frame_id - before.frame_id)
    const points = before.points.map((point, index) => ({ x: point.x + (after.points[index].x - point.x) * ratio, y: point.y + (after.points[index].y - point.y) * ratio }))
    interpolated.push({ ...before, id: `interpolated-${trackId}-${frame}`, points, frame_id: frame, keyframe: false, interpolated: true, locked: true })
  })
...
export default function Workspace({ project: initialProject, onExit }) {
  ...
  useEffect(() => {
    ...
    api.getAnnotation(project.id, currentImage.id).then((data) => {
      if (!cancelled) { setDocument(data); setPast([]); setFuture([]); setSelectedId(null); setCurrentFrame(0); setSaveState('已儲存') }
    }).catch((err) => setError(err.message))
    return () => { cancelled = true }
  }, [project.id, currentImage?.id])
  ...
  const deleteSelected = useCallback(() => {
    ...
    commit((current) => ({ ...current, annotations: current.annotations.filter((item) => item.id !== selectedId) }))
    setSelectedId(null)
  }, [commit, selectedId])
  ...
  const updateSelected = (patch) => commit((current) => ({ ...current, annotations: current.annotations.map((item) => item.id === selectedId ? { ...item, ...patch } : item) }))
  ...
  return (
    <main className="workspace-shell">
      <input ref={fileInputRef} className="hidden-file-input" type="file" multiple accept={project.mediaType === 'video' || project.mediaType === 'both' ? 'image/*,video/*' : 'image/*'} onChange={handleUpload} />
